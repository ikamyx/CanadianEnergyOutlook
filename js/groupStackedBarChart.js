...
function groupStackedBarChart(data, metadata, colors) {
    ...
    const width = 900, height = 600,
          padding = {
            ...
            legend: 30 // space between legend and chart
          },
          legendLineSpace = 20,
    ...
    let maxLegend = 0.2* width - padding.right - legendBoxToText - legendColorBoxWidth;
    let linesCount = 0;
    // legend
    chart.append("g")
    ...
    .each(function(d, i) {
        ...
        // d3.select(this)
        // .attr("transform", function(d, i) {
        //     return `translate(${0}, ${(i + linesCount) * legendLineSpace})`
        // });
        ...
        if (lineWidth > maxLegend) {
            let lines  = Math.round(lineWidth / maxLegend);
            linesCount = lines + linesCount;
            let charMax = Math.floor((d.length / lineWidth) * maxLegend);
            // let textArray = new Array();
            // for(let i=0; i <= lines; i++) {
            //     if(d[(i+1) * charMax]) {
            //         textArray[i] = d.substring(i * charMax, (i+1) * charMax);

                        // do {
                        // let lastChar = textArray[i][textArray[i].length - 1];
                        // textArray[i] = textArray[i].substring(0, textArray[i].length - 1);
                        // d = lastChar + d;
                        // }
                        // while (textArray[i][textArray[i].length - 1] != " ");



                }
            //     else {
            //         textArray[i] = d.substring(i * charMax, d.length); 
            //     }
            // }
    ...
    .attr("transform", `translate(${width*0.8}, ${padding.top})`);
    ...
    let yAxisHeight = height - padding.top - padding.bottom - mainTickMargin - secondTickLineMargin - secondTickMargin - minTickFontHeight * 2;
    ...
    let xAxisWidth = width*0.8 - padding.left - padding.legend - yAxisWidth - yAxisLabelMargin - yAxisLabelHeight - yAxisLineWidth;
    ...
    const preSpacePercent = 0.02,
          barSpacePercent = 0.15,
          groupSpacePercent = 0.13,
