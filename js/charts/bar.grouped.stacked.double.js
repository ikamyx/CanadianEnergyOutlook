...
function bar_grouped_stacked_double(data, metadata, colors, settings, index) {
    ...
    let xAxisWidth = (setting.dimension.width*(setting.distribution.plotRatio/100) - 2 * (setting.yAxis.width + setting.yAxis.labelMargin + setting.yAxis.labelHeight + setting.yAxis.lineWidth) - setting.padding.left - setting.padding.middleX - setting.padding.right) / 2;
    ...
    .each(function(d, i) {
        let j = Math.floor(i / 2) + index;
        d3.select(this)
        .attr("transform", `translate(${setting.dimension.width*(setting.distribution.plotRatio/100)}, ${setting.padding.top + (j - 1) * (setting.padding.middleY + yAxisHeight + setting.xTicks.row1Margin + setting.xTicks.lineSeparatorMargin + setting.xTicks.row2Margin + setting.xTicks.fontHeight * 2 + setting.title.fontHeight + setting.title.marginY)})`);
    })
    ...
    // y axis + label
    /* **************************************************** */
    yAxisInit_bar_double(chart, scaleY, metadata.chart.yLabel, index);
    // /* **************************************************** */
    ...
    /* **************************************************** */
    yAxisGrid_bar_double(chart, xAxisWidth, scaleY, index);
    /* **************************************************** */
    ...
    /* **************************************************** */
    groupPosition_bar_double(chart, barGroupWidth, barGroupPos, setting, distribution, index);
    /* **************************************************** */
    chart.selectAll(`g.row${index} g.plot_groups`)
    .each(function(_, i) {
        d3.select(this)
    ...
    /* **************************************************** */
    ticks_horizontal_bar_double(chart, data, data_, metadata, yAxisHeight, setting, distribution, index);
    /* **************************************************** */
    ...
    /* **************************************************** */
    ticks_2_horizontal_if_ticks_horizontal_bar_double(chart, level_2, metadata, yAxisHeight, xAxisWidth, setting, barGroupWidth, index);
    /* **************************************************** */
    ...
    // add plot title
    /* **************************************************** */
    plotTitle_double(chart, setting, yAxisHeight, xAxisWidth, level_3, index);
    /* **************************************************** */
};