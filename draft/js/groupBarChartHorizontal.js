...
function groupBarChartHorizontal(data, metadata, colors) {
    ...
    let total = data.map(item => item.sum).reduce((prev, next) => prev + next);
    ...
    data_.forEach(e => {
        let temp = e.map(item => item.value).reduce((prev, next) => prev + next);
        e.percent = Math.floor(temp / total * 100);
    })
    ...
    // scale for Y
    let yAxisHeight = height - padding.top - padding.bottom - figureTitleMargin - titleHeight - secondTickLineMargin - secondTickLineMargin - minTickFontHeight;
    ...
    .each(function(d, i) {
        d3.select(this)
        .append("text")
        .text(d => `${d[0][metadata.chart.level_2]} (${d.percent}%)`)
        .attr("x", 0)
        .attr("y", 0)
    })
    ...
    chart.selectAll("g.bar_groups text")
    .each(function(d, i) {
        textSizes.push(d3.select(this).node().getBBox().width);
    })
